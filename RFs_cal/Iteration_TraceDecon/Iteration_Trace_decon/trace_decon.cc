...
Feb, 2015 by Xiaotao Yang
	1. Add functionality to calculate the trace SNR after rotation and before deconvolution.
      ...
      "rawsnr" attribute.
May, 2018 by Cong Li
    1. Fix bugs in seispp, apply the code into a general Antelope database for IRIS data
    2. Make comments
*/
...
int main(int argc, char **argv)
{
	...
	for(int i=2;i<argc;++i)
	{
		...
		else if(!strcmp(argv[i],"-pf"))
		{
			++i;
			if(i>=argc) usage();
			pfin = string(argv[i]); // Parameter file handle
		}
		else if(!strcmp(argv[i],"-o"))
		{
			++i;
			if(i>=argc) usage();
			dbout=string(argv[i]); // Output db handle
		}
...
/**********************************************************************************
 ...
 Note: some object definition can be referenced in http://www.indiana.edu/~pavlab/software/seispp/html
***********************************************************************************/
	Pf *pf;
...
/*****************************************************************************************
                  BUILD A DATABASE SUBSET
    ( dbhm: A view of the joint event->origin->assoc->arrival and then the subset of sta and evid)
    ( dbhv: A view of the joint event->origin)
        // First get all the database components assembled.
		// input data, output data.
*******************************************************************************************/
		DatascopeHandle dbh(dbin,false); // get input database handle
...
/***************************************************************************************
          COLLECT THE RAW DATA, BUILD THREE-COMPONENT GATHER, FILTER AND ROTATION
 The process will work based on every origin in the view of dbhv.
 First, the code will check all stations that record a certain event. (Line: stations = new SeismicArray...)
 Then, the raw data will be collected (Line rawdata=array_get_data...), and built into three-component gather based on
 the view of dbhm (Line regular_gather=BuildRegularGather...)
 ***************************************************************************************/
...
// Start to process every event
		for(record=0;record<recordnum;++record,++dbhv)
		{
            ...
            // Find station assemble during the event time
			// On the first record we need to load the station geometry object
			if(record==0)
			{
           ...
           // Read the raw data using the time window based constructor
            ...
            // Build three-component gather, then apply filter and rotation
