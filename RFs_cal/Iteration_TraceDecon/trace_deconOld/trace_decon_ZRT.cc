		...
		for(d=gather->member.begin(), i=0;d!=gather->member.end();++d,++i)
		{
			try {
				for(int j=0;j<3;j++)
				{
					...
					{
						case 0:
							data->put(string("chan"),string("IE"));
						break;
						case 1:
							data->put(string("chan"),string("IN"));
						break;
						case 2:
							data->put(string("chan"),string("IZ"));
						break;
					}
					data->put("dir",dir);
					data->put("dfile",dfile);
					int rnum;
					if((rnum=dbsave(*data,dbh.db,wfdtable,mdlo,amo))!=-1)
					{
                        rnum++;
						ofs<<rnum<<"	"
						   <<data->get_string("sta")<<"	"
	...
	else
	{
		...
		for(d=gather->member.begin(),i=0;d!=gather->member.end();++d,++i)
		{
			try {
				...
				d->put("wfprocess.algorithm",string("array_decon"));
				int rnum;
