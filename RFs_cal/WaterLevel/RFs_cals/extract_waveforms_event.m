%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
%% Extraction Data for each event from Antelope Database, and save the data in mat format
%    Note: Antelope Database should be prepared    
%   
%% History 
% 5/10/16 Haiying Gao
% 5/19/16 Cong Li, Add output of horizontal direction (including east and north direction)
% 5/24/16 Cong Li, Add output of sample rate, comment the resampling part (that is
% output component is raw data with original sampling rate)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear all; close all;
...
%% ===== extract data from Antelope database =====
 ...
 dir = ['/Users/congli/RFs_Research/RFs_NewEngland/' char(network) '/' char(network) ]
 db = dbopen( dir, 'r' );
...
%for evt = 1
   ...
   arrivaltime=[]; dep=[];Rx=[];Tx=[];Zx=[];MagB=[];Ehang=[];Nhang=[];Zhang=[];delta=[];
   Zcomp={};H1comp={};H2comp={};save_tick=0;
   index11 = find(orid == uorid(evt));  % changed from the origonal program get rid of repeated stations for each event
   [trash, index22] = unique(arsta(index11));
   index = index11(index22);
