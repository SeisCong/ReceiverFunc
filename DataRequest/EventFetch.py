###################################################################################################################
##  Download Event Data
# Searches for events greater than given magnitude and downloads specified components, resamples to specified rate,
# removes instrument response, and saves as SAC file.
#
## C. Li (lic8@sustech.edu.cn/leecong87@gmail.com)
#  Sustech U.
###################################################################################################################
#% load_ext
#autoreload
...
for iev, ev in enumerate(cat_evts):
    ...
    for ista in range(0, len(inventory[0])):
        ...
        if gcarc>=30 and gcarc<=180: # for teleseismic RFs
            # Loop through components
             for icomp, comp in enumerate(comps):
                try:
                     st = client.get_waveforms(network=network, station=station, location="*", channel=comp, starttime=tbeg,
                ...
                if is_removeresp:
                     ...
                     if st[0].stats.response.instrument_sensitivity.input_units == 'PA':
                         st.remove_response(output='VEL', zero_mean=True, taper=True, taper_fraction=0.05,
                                       pre_filt=[0.001, 0.005, sr / 3, sr / 2], water_level=600)
                     else:
                         st.remove_response(output=outunits, zero_mean=True, taper=True, taper_fraction=0.05,
                                       pre_filt=[0.001, 0.005, sr / 3, sr / 2], water_level=600)
                st.trim(starttime=tbeg, endtime=tend, pad=True, nearest_sample=False,
