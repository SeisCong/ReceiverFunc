...
% Get position of box
  ...
  % Draw box
  restrictedParamsHandles = figure('Name','Restricted Params','Units','Pixels','Position',p,'Resize','off','Tag','restrictedParamsWindow');
...
function confirm_call_back(src, evt) 
    hdls = findobj('Tag','passwordEntryEdit');
    pw = hdls.String;
    hdls = findobj('Tag','userNameEntryEdit');
    usr = hdls.String;
    encryptedInfo = cell(2,1);
    encryptedInfo{1} = usr;
    encryptedInfo{2} = pw;
    save('junk.mat','encryptedInfo');
    hdls = findobj('Tag','restrictedParamsWindow');
    close(hdls);
