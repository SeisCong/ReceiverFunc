...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Modification History by Cong Li (congli87@arizona.edu)
%% 1. add a output of original spike time series in iterdecon (change include processRFiter.m and makeRFitdecon.m)
%% 2. Fix bugs for inconsisent hdr.Length or the length is too short
%% 3. Force to skip calculation if the file has been existed already
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    switch params.FileNameConvention
        case 'FuncLabFetch'
            ...
            for isf = 1:size(Stations,1)
                ...
                EastSacFile = dir([ThisDir '*' StationName '*E.*.SAC']); %HE % modify by Cong
                NorthSacFile = dir([ThisDir '*' StationName '*N.*.SAC']); %HN
                VerticalSacFile = dir([ThisDir '*' StationName '*Z.*.SAC']); %HZ
...
overwrite=0; % Temporary paramter; It can be added into params in UI later; add by Cong
for idir=1:size(DataDirs,1)
    ...
    for istation=1:length(DataStructure(idir).Stations)
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Check the exist files and decide to skip or overwrite; add by Cong
        % this only works for calculation of both r and r RFs now, update
        % will be made later!
        if overwrite==0
        ...
        rfilelead = sprintf('*_%s_%1.1f.%s.',DataStructure(idir).Stations{istation}, rfOptR.F0, deconType);
        tfilelead = sprintf('*_%s_%1.1f.%s.',DataStructure(idir).Stations{istation}, rfOptT.F0, deconType); 
        spfilelead = sprintf('*_%s.%s.',DataStructure(idir).Stations{istation}, deconType);
        if (length(dir([outputDir rfilelead 'eqr']))==1 && length(dir([outputDir tfilelead 'eqt']))==1 &&...
           length(dir([outputDir spfilelead 'spr']))==1 && length(dir([outputDir spfilelead 'spt']))==1 &&...
           length(dir([outputDir rfilelead 'r']))==1 && length(dir([outputDir rfilelead 't']))==1 && ...
           length(dir([outputDir rfilelead 'z']))==1)
         
         eqrfilename=dir([outputDir rfilelead 'eqr']);
         eqtfilename=dir([outputDir tfilelead 'eqt']);
         rfilename=dir([outputDir rfilelead 'r']);
         tfilename=dir([outputDir rfilelead 't']);
         zfilename=dir([outputDir rfilelead 'z']);

         DataStructure(idir).EQRFiles(istation).name = eqrfilename.name; 
         DataStructure(idir).EQTFiles(istation).name = eqtfilename.name;
         DataStructure(idir).EQRLead(istation).name  = eqrfilename.name(1:end-3);
         DataStructure(idir).EQTLead(istation).name  = eqtfilename.name(1:end-3);

         DataStructure(idir).RFiles(istation).name   = rfilename.name;
         DataStructure(idir).TFiles(istation).name   = tfilename.name;
         DataStructure(idir).ZFiles(istation).name   = zfilename.name;
         continue;
        % else
        %     disp('Error, need to debug for overwrite!');
        %     return
        end
        clear deconType rfilelead tfilelead spfilelead eqrfilename eqtfilename rfilename tfilename zfilename
        end
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        % Check if the length of waveform is consistent with the
        % requirment, add by Cong
        if length(eseis)~=hdr.trcLen || length(nseis)~=hdr.trcLen || length(zseis)~=hdr.trcLen
            disp([DataStructure(idir).Stations{istation} ', hdr length is not consistent with the waveform length, skip!']);
            continue;
        end
  
        % check if the length of waveform is consistent with the
        % requirement, add by Cong
        if hdr.times.e-hdr.times.b<=(params.EQRRawParams.TBEFORE+params.EQRRawParams.TAFTER)/2
             disp([DataStructure(idir).Stations{istation} ', the waveform length is less than half of required time segment, skip!']);
             continue;
        end
        ...
        if skipFlagR == 0
            ...
            if ierr == 0
                ...
                if strcmp(params.EQRDeconType,'Iterative')
                    sprfilelead = sprintf('%s_%s.%s.',network, station, deconType);
                    sprfilename = sprintf('%sspr',sprfilelead); % add by Cong
                end
                ...
                if skipFlagT == 1
                    ...
                    if strcmp(params.EQRDeconType,'Iterative')
                        sptfilename = sprintf('%sspt',sprfilelead); % add by Cong
                        fullOutFileName = fullfile(outputDir, sptfilename);
                        writeSAC(fullOutFileName, rfhdr, 0*rspikes);
                    end
                end
                ...
                if strcmp(params.EQRDeconType,'Iterative') %% add by Cong
                     fullOutFileName = fullfile(outputDir, sprfilename);
                     writeSAC(fullOutFileName, rfhdr, rspikes);
                end
        ...
        if skipFlagT == 0
            ...
            if ierrT == 0
                ...
                switch params.EQTDeconType
                    case 'Iterative'
                        [rftime, rfseis,rspikes, rfhdr, ierr] = processRFiter(numer, denom, hdrT, rfOptT , false);
                        %deconType = sprintf('iterative_%03d',rfOpt.ITERMAX);
                        if ierr == 1
                ...
                if strcmp(params.EQRDeconType,'Iterative')% add by Cong
                    sptfilelead = sprintf('%s_%s.%s.',network, station, deconType);
                    sptfilename = sprintf('%sspt',sptfilelead); 
                end
                ...
                if skipFlagR == 1
                    ...
                    if strcmp(params.EQRDeconType,'Iterative')
                        sprfilename = sprintf('%sspr',sptfilelead); % add by Cong
                        fullOutFileName = fullfile(outputDir, sprfilename);
                        writeSAC(fullOutFileName, rfhdr, 0*rspikes);
                    end
