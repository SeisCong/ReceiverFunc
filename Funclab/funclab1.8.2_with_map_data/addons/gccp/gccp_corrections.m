function gccp_corrections(cbo,eventdata,handles)
%GCCP_CORRECTIONS Velocity heterogeneity corrections
%   GCCP_CORRECTIONS is a callback function that computes the differential
...
%--------------------------------------------------------------------------
temp = who('-file',GCCPMatFile,'MidPoints*');
for n = 1:length(temp)
    Numb = num2str(n,'%02.0f');
    load(GCCPMatFile,['MidPoints_' Numb])
    ...
    save(GCCPMatFile,['TimeCorrections_' Numb],'-append');
...
CTime = clock;
NewLog{1} = sprintf('%02.0f/%02.0f/%04.0f %02.0f:%02.0f:%02.0f - GCCP Stacking: Velocity Heterogeneity Corrections set %s',CTime(2),CTime(3),CTime(1),CTime(4),CTime(5),CTime(6), SetName);
...
close(WaitGui)
set(h.MainGui.message_t,'String',['Message: GCCP Stacking: 3D Timing Corrections took ' num2str(t_temp) ' seconds (or ' num2str((t_temp)/60) ' minutes).'])